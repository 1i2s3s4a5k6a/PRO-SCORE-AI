...
# --- 3. MARKET SENTIMENT ---
    ...
    try:
        ...
        for i in n_res['items'][:6]:
            polarity = TextBlob(i['title']).sentiment.polarity
            mood = "🟢 POSITIVE" if polarity > 0.1 else "⚪ NEUTRAL"
...
# --- 4. H2H ANALYSIS ---
    ...
    try:
        ...
        if st.button("RUN"):
            ...
            if 'matches' in res and res['matches']:
                data = [{"Date": m['utcDate'][:10], "Score": f"{m['score']['fullTime']['home']}-{m['score']['fullTime']['away']}"} for m in res['matches']]
                st.table(pd.DataFrame(data))
            else:
                st.warning("No H2H matches found.")
    except:
